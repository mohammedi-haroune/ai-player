...
class Predictor(handler: ActorRef) extends Actor {
  ...
  override def receive: Receive = {
    LoggingReceive {
      case input: Input =>
        val gesture = predict(input)
        handler ! gesture
    }
  }

  def predict(input: Input): Gesture = {
    val out = model.output(Nd4j.create(input.toArray)).toDoubleVector
...
object Predictor {
  def props(handler: ActorRef): Props = Props(new Predictor(handler))
}
